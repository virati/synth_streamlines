...
class bundle:
    ...
    def generate_streamlines(self, a, b, N, noise_level, tortuosity):
        ...
        for _ in range(N):
            ...
            streamlines.append((x_spline, y_spline))
        return streamlines

    def plot_streamlines(self, streamlines):
        for x, y in streamlines:
            plt.plot(x, y)
        plt.xlabel("X")
        plt.ylabel("Y")
        plt.title("Generated Streamlines")
        plt.show()
...
class StreamlineChecker:
    ...
    def check_streamlines(self, bundle, x, y, r):
        self._probe_point = (x, y, r)
        passing_streamline_indices = []
        for ss, (x_spline, y_spline) in enumerate(bundle.streamlines):
            distances = np.sqrt((x_spline - x) ** 2 + (y_spline - y) ** 2)
